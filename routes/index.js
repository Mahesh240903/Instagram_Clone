...
router.get("/like/:postid", async function (req, res) {
  const post = await postModel.findOne({ _id: req.params.postid }).populate("user").populate("like");
  const user = await userModel.findOne({ username: req.session.passport.user });
  const alreadyLiked = post.like.some(like => like.equals(user._id));
  if (!alreadyLiked) {
    // If not already liked, add the user's ID to the 'like' array
    post.like.push(user._id);
  } else {
    // If already liked, remove the user's ID from the 'like' array
    post.like = post.like.filter(like => !like.equals(user._id));
  }
...
router.get("/saved",isLoggedIn,async function(req,res){
  ...
  res.render("savedprofile",{ footer: true, user:user });
})
